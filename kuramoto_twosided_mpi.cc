...
int main(int argc, char const* argv[]) {
  ...
  double alpha = argc > 7 ? atof(argv[7]) : 2.0;   // stability
  double lambda = argc > 8 ? atof(argv[8]) : 0.0;  // tempering
  double sigma = argc > 9 ? atof(argv[9]) : 0.5;   // diffusivity
  double K = argc > 10 ? atof(argv[10]) : 1.0;       // coupling
  double max_t = argc > 11 ? atof(argv[11]) : 2.0;
  double c = argc > 12 ? atof(argv[12]) : 1.1;
  ...
  while (getline(infile, line)) {
    ...
    if (alpha > 1.999) {
      stats =
          paths(G, rnorm, beta, alpha, a, b, kappa, max_t, nsteps, npaths_rank, seed);
    } else {
      if (lambda < 0.001) {
        stats = paths(G, rstable, beta, alpha, a, b, kappa, max_t, nsteps,
                      npaths_rank, seed);
      } else {
        stats = paths(G, rtstable, beta, alpha, a, b, kappa, max_t, nsteps,
                      npaths_rank, seed);
      }
    }
