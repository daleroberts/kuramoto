...
inline Complex ctstable(double y, double alpha, double a, double b) {
  Complex I(0, 1);
  Complex A = a * tgamma(-alpha) * (pow(b - I * y, alpha) - pow(b, alpha) +
                                    I * y * alpha * pow(b, alpha - 1));
  return exp(A);
}
...
double ptstable(double x, double alpha, double a, double b) {
  ...
  if (alpha < 1) {
    ...
    m2 = a * pow(b, alpha - 2) *
         (a * pow(b, alpha) * pow(tgamma(1 - alpha), 2) + tgamma(2 - alpha));
  } else {
    m1 = 0.0;
...
double qtstable(double p, double alpha, double a, double b) {
  auto f = [=](double x) { return ptstable(x, alpha, a, 0.001) - p; };
  ...
  auto r = boost::math::tools::bracket_and_solve_root(f, 0.5, 2.0, true, tol, maxiter);
